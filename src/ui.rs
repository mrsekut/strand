...
fn draw_standalone_issue_detail(frame: &mut Frame, app: &App) {
    ...
    draw_standalone_issue_keybar(frame, app, chunks[1]);
    draw_notification(frame, app, chunks[2]);
}

// --- Child Detail (epic配下のissue) ---

fn draw_child_detail(frame: &mut Frame, app: &App) {
    let issue_id = match &app.view {
    ...
    draw_child_keybar(frame, app, chunks[1]);
    draw_notification(frame, app, chunks[2]);
}
...
fn draw_standalone_issue_keybar(frame: &mut Frame, app: &App, area: Rect) {
    use crate::app::{ConfirmAction, InputMode};
...
fn draw_child_keybar(frame: &mut Frame, app: &App, area: Rect) {
    use crate::app::{ConfirmAction, InputMode};
